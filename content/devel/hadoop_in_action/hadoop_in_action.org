...
1. 应用程序的输入必须被构建成包含pair（key/value）的list，list(k1,v1)。
   处理多个文件的输入格式通常为 =list(<String filename, String file_content>)= 。处理单个大文件的格式可能是 =list(<Integer line_number, String log_event>)= 。
